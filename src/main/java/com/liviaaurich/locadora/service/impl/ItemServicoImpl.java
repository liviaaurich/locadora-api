...
public class ItemServicoImpl implements ItemServico {
    ...
    @Override
    public Page<ItemDTO> obterTodos(ItemFiltro filtro, Pageable pageable) {
        Page<Item> resultado = this.itemRepository.findAll(filtro.filter(), pageable);

        return resultado.map(itemMapper::toDto);
    }
}
